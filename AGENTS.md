...
- **"First, Make It Work. Then, Make It Right.":** Prioritize functional code, followed by relentless refactoring and cleaning.
- **The Boy Scout Rule:** Continuously leave code cleaner than it was found.
- **Minimize Complexity:** Strive for the simplest design possible, prioritizing reader understanding by untangling high-level policies from low-level details.
- **Continuous Design:** Design is an ongoing activity; each change updates the system's design. Apply design principles throughout development.

## Approved Architectural Patterns

Clean Architecture (Mandatory Layers): Adhere to Entities, Use Cases, Interface Adapters, and Frameworks/Drivers layers. The Dependency Rule mandates that source code dependencies only point inward.
...
- **DIP (Dependency Inversion Principle):** High-level modules depend on abstractions, not low-level modules.
- **Hexagonal Architecture (Ports and Adapters):** Wrap external system interactions with application-centric interfaces (ports) and implement them with adapters to isolate core logic.

Component Cohesion (REP, CCP, CRP):

- **REP (Reuse/Release Equivalence Principle):** Group reusable classes/modules into releasable components.
- **CCP (Common Closure Principle):** Group modules that change together for the same reasons.
- **CRP (Common Reuse Principle):** Do not force users to depend on unneeded things.
- **Acyclic Dependencies Principle (ADP):** The component dependency graph must not contain cycles.
- **"Tell, Don't Ask" (Law of Demeter):** Modules should not know about the innards of objects they manipulate.
- **Composition over Inheritance:** Favor composition of smaller widgets/classes over deep inheritance hierarchies.

## Code Structure & Readability (Flutter/Dart Specifics)

- **Small Functions/Methods:** Functions and methods must be very small, doing "one thing."
- **The Stepdown Rule:** Organize methods in a class with public methods followed by their private helper methods, creating a top-down narrative.
- **One Level of Abstraction per Function:** All statements within a function must be at the same abstraction level.
- **Avoid Entanglement:** Minimize dependencies between conceptually distinct functions.
- **Minimize Arguments:** Aim for zero, one, or at most two/three arguments; use custom objects for more. Avoid flag arguments.
- **Avoid Output Arguments:** Functions should return values; use records or custom objects for multiple return values.
- **Replace Switch Statements with Polymorphism:** Replace type-code-based switch statements with polymorphic solutions.
- **Vertical Formatting:**
  File Size: Keep Dart files small (under 200-300 lines, max 500). One class/mixin/enum/extension per file is preferred.
  Openness: Separate logical concepts with blank lines.
  Density: Tightly related lines should be vertically dense.
  Distance: Keep conceptually related code vertically close.
  Dependent Functions: Calling functions should ideally appear above called functions.
- **Horizontal Formatting:**
  Line Width: Limit lines to a maximum of 120 characters.
  Openness/Density: Use horizontal whitespace to associate/disassociate elements.
  Indentation: Use 2 spaces for indentation in Dart.
- **DRY (Don't Repeat Yourself):** Eliminate essential duplication by refactoring into reusable components.
...
### Embrace `ThemeData` and Material design
...
- **Asymptotic 100% Test Coverage:** Strive for 100% line and branch coverage as a continuous improvement goal.
- **F.I.R.S.T. Principles: All tests must be:**
  Fast
  Isolated
  Repeatable
  Self-Validating
  Timely
- **Single Act Rule:** Each test should assert one logical concept, triggered by one action.
- **Clean Tests:** Test code must be clean, readable, well-structured, and maintainable.
- **Domain-Specific Testing Language:** Develop testing APIs and helper functions to make tests expressive.
- **Continuous Build & Testing:** Run all tests automatically and frequently; the build must remain green.
- **Acceptance Testing:** Define requirements as automated acceptance tests, involving BAs and QAs.
...
- **Screen Reader Testing:** Regularly test your app with TalkBack (Android) and
  VoiceOver (iOS).
- **Accessibility Text:** Use text that screen readers (TalkBack, VoiceOver, JAWS) can read aloud, including both visible and nonvisible alternative text.
- **Adjacent Text:** Ensure text in and around images presents key information about the image to make it more accessible.
- **Captions:** Provide text below images to explain contextual information. This helps both sighted and screen reader users understand how the image relates to the content.
- **Embedded Text:** Avoid embedding essential text in imagery as screen readers cannot read it. If unavoidable, include the essential information in the alt text.
- **Alternative Text (Alt Text):**
  - Provide a short label (up to 125 characters) that describes an image for users unable to see it.
  - Do not use phrases like "image of" or "picture of".
  - Include targeted keywords to inform the user and improve SEO.
  - Alt text also serves as a fallback for sighted users if an image fails to load.

## Refactoring & Continuous Improvement

- **Relentless Refactoring:** Continuously refactor code to improve structure, clarity, and maintainability, trusting the test suite.
- **Continuous Design:** Regularly evaluate design against the "Four Cs" (Clarity, Conciseness, Confirmability, Cohesion) and make incremental improvements.
- **Small Cycles:** Work in very short development cycles (minutes); commit frequently. Continuous integration and deployment are ultimate goals.

## Methodology: Material Expressive Design

**Principle:** Learn Before, Not After.

This methodology enforces a proactive approach to design, ensuring that systems are robust, inclusive, and adaptable from inception.

### 1. Phase 1: Pre-emptive Analysis (The Foundation)

- **Objective:** Pre-emptively reduce design biases and ensure universal access.
- **Action:** Before any implementation begins, conduct a formal analysis of diverse user abilities, life experiences, and environmental contexts.
- **Requirement:** Develop "User Spectrum" profiles that explicitly include disability, varying technical literacy, and non-standard interaction modalities (e.g., switch control, voice only, screen readers).
- **Outcome:** A definition of the design space where no user group is treated as an "edge case," but rather as a primary stakeholder in the core experience.

### 2. Phase 2: Generative Constraints (The Framework)

- **Objective:** Transform mandates into innovation.
- **Protocol:** Interpret all core design constraints—specifically ethical guidelines, functional requirements, and accessibility mandates (WCAG)—strictly as **generative opportunities** rather than compliance barriers.
- **Mindset Shift:**
  - _Compliance:_ "We must ensure high contrast to pass the audit."
  - _Generative:_ "High contrast improves readability for everyone in bright sunlight. We will use high-contrast typography as a core aesthetic pillar to define our brand's bold identity."
- **Mechanism:** Every architectural decision must explicitly cite which constraint it leverages to create a superior universal experience.

### 3. Phase 3: Iterative Recovery (The Loop)

- **Objective:** Continuous refinement through observability.
- **Protocol:** Treat unexpected product outcomes, "misuse," or user-reported problems as **essential research foundations**.
- **Action:**
...
- Dart with Flutter 3.10+ + go_router (declarative navigation with typed routes), flutter_riverpod (state management), hive (foldable posture persistence), Material 3 widgets (001-adaptive-layout-system)
